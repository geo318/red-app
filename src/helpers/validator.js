export const validator = (inputs, values) => {
  const filterArray = inputs.filter(e => e.required === true && e.name)
  let namesArray = filterArray.map(e => e.name)
  let patternsArray = filterArray.map(e => e?.error?.pattern ? e.error.pattern : e.requried ? /.+/ : /./)
  ...
  if(!test_1 || !test_2) return true //error
